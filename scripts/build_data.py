            ...
            for name in z.namelist():
                ...
                with z.open(name) as f:
                    ...
                    for original in csv.DictReader(io.StringIO(text),dialect=dialect):
                        ...
                        try:
                            # RUIAN CSV publishes S-JTSK X/Y as positive magnitudes.
                            # EPSG:5514 uses negative East/North values for this area, so negate both.
                            # With always_xy=True pyproj expects (easting, northing) = (Y, X).
                            lon,lat=tr.transform(-float(y.replace(',','.')),-float(x.replace(',','.')))
                            result[k]=(lat,lon,row)
